...
template<class T> using pqm = priority_queue<T>;                          // Max-heap (default)
...
#define fastio ios::sync_with_stdio(false); cin.tie(nullptr); cout.tie(nullptr);  // Fast I/O
...
inline str readline() { str s; getline(cin >> ws, s); return s; }

// Read vector of size n (or full if n=-1)
template<class T> inline void readvec(vector<T> &v, int n = -1) {
 if (n == -1) n = sz(v);
 rep(i,0,n) cin >> v[i];
}
...
template<class T> inline void print(const T &x, bool nl = true) {
 cout << x; if (nl) cout << endl;
}

// Print vector with custom separator
template<class T> inline void printvec(const vector<T> &v, char sep = ' ', bool nl = true) {
...
// Print 2D vector
template<class T> void print2d(const vector<vector<T>>& g) {
