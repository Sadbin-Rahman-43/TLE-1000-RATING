...
using vpll = vector<pll>;    // Vector of pairs
...
void solve()
{
    // MY ATTEMPT TO WRITE A READABLE CODE
    ll numberofpeople, initialcost, totalcost = 0;

    read(numberofpeople, initialcost);

    ll numberofpeopleremaining = numberofpeople - 1;

    totalcost = initialcost;

    vl vectorofpeople(numberofpeople), vectorofcost(numberofpeople);

    readvec(vectorofpeople);

    readvec(vectorofcost);
    ...
    vpll vectorofcost_vectorofpeople;
    for (int i = 0; i < numberofpeople; i++)
    {
        vectorofcost_vectorofpeople.pb({vectorofcost[i], vectorofpeople[i]});
    }
    sb(vectorofcost_vectorofpeople);
    for (ll i = 0; i < numberofpeople; i++)
    {
        if (vectorofcost_vectorofpeople[i].ss < numberofpeopleremaining)
        {
            totalcost += min(initialcost,vectorofcost_vectorofpeople[i].ff) * vectorofcost_vectorofpeople[i].ss;
            numberofpeopleremaining -= vectorofcost_vectorofpeople[i].ss;
        }
        else
        {
            totalcost +=  min(initialcost,vectorofcost_vectorofpeople[i].ff)* numberofpeopleremaining;

            numberofpeopleremaining = 0;
        }
    ...
    // BRUTEFORCE
    // while (numberofpeopleremaining >= 0)
    // {

    //     int mincost = min_element(all(vectorofcost)) - vectorofcost.begin();

    //     if (vectorofpeople[mincost] <= numberofpeopleremaining)
    //     {
    //         totalcost += vectorofpeople[mincost] * vectorofcost[mincost];

    //         numberofpeopleremaining -= vectorofpeople[mincost];
    //         vectorofcost[mincost] = INFi;
    //     }

    //     else
    //     {
    //         totalcost += numberofpeopleremaining * vectorofcost[mincost];
